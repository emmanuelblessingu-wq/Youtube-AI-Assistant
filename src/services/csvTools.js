...
export const YOUTUBE_TOOL_DECLARATIONS = [
  ...
  },
  {
    ...
    description:
      'Plot a numeric metric (view_count, like_count, comment_count, duration) against release_date for the loaded YouTube channel videos. ' +
      'Call when user asks to plot, graph, chart, or visualize views/likes/comments over time.',
    parameters: {
      type: 'OBJECT',
      properties: {
        metric: {
          type: 'STRING',
          description: 'One of: view_count, like_count, comment_count, duration',
        },
        title: { type: 'STRING', description: 'Chart title' },
        color: { type: 'STRING', description: 'Chart line color (hex)' },
      },
      required: ['metric'],
    },
  },
  {
    name: 'play_video',
    description:
      'Display a clickable video card for a YouTube video. Call when user says "play", "open", "watch", or "show me" a video. ' +
      'Match by title keywords, ordinal (first, second, most viewed), or "most viewed".',
    parameters: {
      type: 'OBJECT',
      properties: {
        ...
        description: { type: 'STRING', description: 'Brief reason for showing this video' },
      },
  ...
  },
  {
    ...
    description:
      'Compute mean, median, std, min, max for a numeric field in the YouTube channel JSON. ' +
      'Call when user asks for statistics, average, distribution of view_count, like_count, comment_count, or duration.',
    parameters: {
      type: 'OBJECT',
      properties: {
        field: {
...
// Execute YouTube tools client-side
export const executeYoutubeTool = (toolName, args, channelData) => {
  switch (toolName) {
    case 'generateImage': {
    ...
    case 'plot_metric_vs_time': {
      if (!channelData?.videos?.length) return { error: 'No channel data loaded.' };
      const metric = args.metric || 'view_count';
      const plotData = channelData.videos
        .filter((v) => v[metric] !== undefined && v.release_date)
        .map((v) => ({ date: v.release_date, value: v[metric], title: v.title }))
        .sort((a, b) => new Date(a.date) - new Date(b.date));
      return {
        _toolType: 'chart',
        metric,
        title: args.title || `${metric.replace(/_/g, ' ')} over Time`,
    ...
    case 'play_video': {
      return {
        ...
        title: args.title,
        thumbnail_url: args.thumbnail_url || `https://img.youtube.com/vi/${args.video_id}/hqdefault.jpg`,
    ...
    case 'compute_stats_json': {
      ...
      const values = channelData.videos
        .map((v) => parseFloat(v[field]))
        .filter((v) => !isNaN(v));
      if (!values.length) return { error: `No numeric values found for field "${field}"` };
      ...
      return {
        _toolType: 'stats',
        field,
        label: args.label || field.replace(/_/g, ' '),
        stats: {
